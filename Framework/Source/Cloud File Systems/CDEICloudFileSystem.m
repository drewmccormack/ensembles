...
- (void)repairEnsembleDirectory:(NSString *)ensembleDir completion:(CDECompletionBlock)completion
{
    ...
    // This repair looks for duplicates of the ensemble directory. These are generated by
    ...
    [operationQueue addOperationWithBlock:^{
        ...
        NSArray *directoryPathsToMerge = [self rootDirectoriesRequiringMergeForEnsembleDirectory:fullEnsembleDirPath error:&error];
        ...
        error = [self mergeEnsembleDirectory:fullEnsembleDirPath withDirectories:directoryPathsToMerge];
        [self dispatchCompletion:completion withError:error];
    }];
}

- (NSError *)mergeEnsembleDirectory:(NSString *)ensembleDirPath withDirectories:(NSArray *)dirPathsToMerge
{
    for (NSString *dirToMerge in dirPathsToMerge) {
        ...
        while ( relativePath = [dirEnum nextObject] ) {
            NSString *ensemblePath = [ensembleDirPath stringByAppendingPathComponent:relativePath];
            NSString *duplicatePath = [dirToMerge stringByAppendingPathComponent:relativePath];
            if ([fileManager fileExistsAtPath:ensemblePath]) continue;
            
            NSURL *ensembleURL = [NSURL fileURLWithPath:ensemblePath];
            ...
            if ([dirEnum.fileAttributes.fileType isEqualToString:NSFileTypeDirectory]) {
                [coordinator coordinateWritingItemAtURL:ensembleURL options:0 error:&fileCoordinatorError byAccessor:^(NSURL *newURL) {
                    NSError *error = nil;
                    success = [fileManager createDirectoryAtURL:newURL withIntermediateDirectories:YES attributes:nil error:&error];
                    if (!success) fileManagerError = error;
                }];
            }
            else if ([dirEnum.fileAttributes.fileType isEqualToString:NSFileTypeRegular]) {
                [coordinator coordinateReadingItemAtURL:duplicateURL options:0 writingItemAtURL:ensembleURL options:NSFileCoordinatorWritingForReplacing error:&fileCoordinatorError byAccessor:^(NSURL *newReadingURL, NSURL *newWritingURL) {
                    NSError *error = nil;
        ...
        __block NSError *fileManagerError = nil;
        [coordinator coordinateWritingItemAtURL:dirToMergeURL options:NSFileCoordinatorWritingForDeleting error:&coordinatorError byAccessor:^(NSURL *newURL) {
            NSError *error = nil;
...
- (NSArray *)rootDirectoriesRequiringMergeForEnsembleDirectory:(NSString *)ensembleDir error:(NSError * __autoreleasing *)returnError
{
    NSMutableArray *directoriesForMerging = [[NSMutableArray alloc] init];
    NSString *rootDir = [ensembleDir stringByDeletingLastPathComponent];
    NSString *ensembleName = [ensembleDir lastPathComponent];
    for (NSUInteger i = 2; i < 10; i++) {
        ...
        NSFileCoordinator *coordinator = [[NSFileCoordinator alloc] initWithFilePresenter:self];
        NSError *error = nil;
        [coordinator coordinateReadingItemAtURL:duplicateURL options:NSFileCoordinatorReadingWithoutChanges error:&error byAccessor:^(NSURL *newURL) {
            BOOL isDir;
            if ([fileManager fileExistsAtPath:newURL.path isDirectory:&isDir] && isDir) {
                [directoriesForMerging addObject:newURL.path];
            }
        }];
        if (error) {
            *returnError = error;
